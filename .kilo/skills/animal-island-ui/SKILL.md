...
> **本地源码参考**：完整仓库已克隆到本 skill 目录下的 `repo/` 子目录（即与本文档同级的 `animal-island-ui/repo/`）。
> 下文所有 `src/components/...`、`demo/...`、`src/styles/...` 路径均为相对 `repo/` 根目录的相对路径，实际查阅时请加 `repo/` 前缀（如 `repo/src/components/Button/`）。
> 同时 `repo/` 下还包含 `AI_USAGE.md` / `DESIGN_PROMPT.md` / `PROMPT.md` / `README.md` / `CHANGELOG.md` 等完整文档，以及 `demo/` 完整 Demo 源码与 `test/` 测试用例，编程时可一并参考。
